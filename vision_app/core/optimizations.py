...
------------------------------------------
- Aplica optimizaciones genéricas de sistema (CUDA/OpenCV/PyTorch) y del SO.
- Funcionalidades principales:
  ...
  * `apply_system_optimizations()`: optimizaciones del sistema operativo:
    ...
    - Memoria: limpia caché del sistema
    - GPU: optimizaciones específicas de Jetson (nvpmodel, jetson_clocks)
    - Red: optimiza buffers de red
  * `get_system_info()`: obtiene información del sistema (CPU, memoria, disco, Jetson)
  * `set_process_priority()`: establece prioridad del proceso
- Se llama desde `vision_app/app.py` en la inicialización.
"""
...
def apply_all(context) -> None:
    """Aplica optimizaciones genéricas del sistema (CUDA, PyTorch, OpenCV).
    
    Esta función se llama automáticamente al inicializar la app.
    """
    import torch
    import cv2
