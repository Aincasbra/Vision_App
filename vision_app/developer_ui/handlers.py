"""
Acciones de la UI de desarrollador
----------------------------------
- Lógica de acciones del panel (RUN/STOP/INFO/CONFIG/AWB/AUTO_CAL/RECORD_60S).
- Es invocado por `developer_ui/app_controller.py`.
"""
import time
import threading
import cv2
...
def handle_action(app, action):
    ...
    if action == "STOP":
        try:
            # Restaurar ROI original antes de parar (solo GenICam/Aravis)
            try:
                # Restaurar tamaño completo de la cámara si el backend lo soporta
                if hasattr(app.camera, 'restore_full_frame'):
                    w_max, h_max = app.camera.restore_full_frame()
...
def _show_info_window(camera):
    ...
    def safe_get(cam, name, default=None):
        try:
            return CameraBackend.safe_get(cam, name, default)
        except Exception:
            return default
...
def _show_config_window(camera, app=None):
    ...
    try:
            ...
            def safe_set(cam, name, value):
                try:
                    return CameraBackend.safe_set(cam, name, value)
                except Exception:
                    return False
