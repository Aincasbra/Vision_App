...
check_logs() {
    ...
    for dir in "${log_dirs[@]}"; do
        if [[ -d "/var/log/vision_app/$dir" ]]; then
            print_success "✓ Directorio /var/log/vision_app/$dir existe"
        else
            print_error "✗ Directorio /var/log/vision_app/$dir no existe"
            return 1
        fi
    done
    
    # Verificar permisos
    local owner=$(stat -c '%U:%G' /var/log/vision_app 2>/dev/null || echo "unknown")
    ...
    local log_files=(
        "/var/log/vision_app/vision/vision_log.csv"
        "/var/log/vision_app/timings/timings_log.csv"
    )
...
check_rsyslog() {
    ...
    if [[ -f "/etc/rsyslog.d/50-vision-app.conf" ]]; then
...
check_logrotate() {
    ...
    # Verificar archivo de configuración
    if [[ -f "/etc/logrotate.d/vision-app" ]]; then
...
check_running_process() {
    ...
    if [[ "$process_count" -gt 0 ]]; then
        ...
        print_warning "⚠ No se encontró proceso Vision App ejecutándose"
    fi
...
check_realtime_logs() {
    ...
    local log_file="/var/log/vision_app/system/system.log"
...
test_reboot() {
    ...
    echo "   Esta prueba reiniciará el equipo y verificará que Vision App arranque automáticamente."
...
show_summary() {
    ...
    # Logs recientes
    echo "📝 Logs recientes:"
    if [[ -f "/var/log/vision_app/vision/vision_log.csv" ]]; then
        echo "Últimas 5 líneas de vision_log.csv:"
        tail -5 /var/log/vision_app/vision/vision_log.csv
    else
    ...
    if [[ -f "/var/log/vision_app/system/system.log" ]]; then
        echo "📊 Logs del sistema (última entrada):"
        tail -1 /var/log/vision_app/system/system.log
    fi
    echo ""
    ...
    df -h /var/log/vision_app 2>/dev/null || echo "Directorio de logs no encontrado"
    echo ""
}
...
main() {
    echo "============================================================================="
    echo "🔍 VERIFICACIÓN POST-INSTALACIÓN VISION APP"
    echo "============================================================================="
    echo ""
    ...
    if [[ $errors -eq 0 ]]; then
        ...
        echo "  systemctl status vision-app.service"
        echo "  tail -f /var/log/vision_app/system/system.log"
        echo "  sudo journalctl -u vision-app.service -f"
        echo ""
    else
        echo "============================================================================="
